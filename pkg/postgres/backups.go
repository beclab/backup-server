...
type Backup struct {
	...
	Location     map[string]string      `gorm:"type:jsonb;not null;default:'{}'"`
	BackupType   map[string]string      `gorm:"type:jsonb;not null;default:'{}'"`
	BackupPolicy *BackupPolicy          `gorm:"type:jsonb;not null;default:'{}'"`
...
func GetBackupByID(backupId string) (*Backup, error) {
	if DBServer == nil {
		return nil, fmt.Errorf("database connection not initialized")
	}

	var backup Backup
	result := DBServer.Where("backup_id = ?", backupId).First(&backup)
	if result.Error != nil {
